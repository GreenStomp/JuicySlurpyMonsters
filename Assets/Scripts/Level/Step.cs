...
public class Step
{
    ...
    /// <summary>
    /// Calculates a simplified next step data. No Special platform event will be triggered
    /// </summary>
    ...
    public bool CalculateNextStep(float totalMovement, Data data)
    {
        if (!data.Plat)
        ...
        if (data.IsSwitchingLanes)
            return SwitchingLaneNextStep(totalMovement, data);
        else
            return NormalNextStep(totalMovement, data);
    }
    /// <summary>
    ...
    /// <summary>
    /// Resets Step.Data structure
    /// </summary>
    /// <param name="data">data to reset</param>
    /// <param name="platform">platform on which to position data</param>
    /// <param name="resetStepDataStructure">true to reset data structure, false to reset only platform</param>
    /// <param name="resetStats">true to reset stats (if present)</param>
    public void Reset(Data data, Platform plat, bool resetStepDataStructure = true, bool resetStats = true)
    {
        data.Plat = plat;
    ...
    private bool NormalNextStep(float totalMovement, Data data)
    {
        Lane lane = data.Plat.Lanes[data.CurrentLane];
        ...
        if (newPercentage > 1f)
        {
            ...
            CalculateNextStep(overMovement, data);
            return true;
        }
    ...
    private bool SwitchingLaneNextStep(float totalMovement, Data data)
    {
        Lane firstLane = data.Plat.Lanes[data.CurrentLane];
        Lane secondLane = data.Plat.Lanes[data.DestinationLane];
