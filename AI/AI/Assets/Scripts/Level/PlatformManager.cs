...
public sealed class PlatformManager : MonoBehaviour
{
    ...
    public Platform FirstPlatform2;
    ...
    void Update()
    {
        ...
        if (deathPlane.DeathPlane.SameSide(platforms.Peek().EndLocation.position, deathPlane.DeathSidePoint))
        {
            ...
            PoolManager.Recycle(platforms.Dequeue());
    ...
    public void ChangeLevel(int nextIndex, bool clearPreviousLevelPools = false)
    {
        ...
        {
            PoolManager.InitializePool(normalPrefabs[i]);
        }
        for (int i = 0; i < specialPrefabs.Length; i++)
        {
            PoolManager.InitializePool(specialPrefabs[i]);
        }
